...
dirLight.position.set(20, 20, 10);  // Upper right position
scene.add(dirLight);
...
// Debug mode control: allow manual progress override
let useManualProgress = false;
let manualProgressValue = progress;
...
renderer.setAnimationLoop(() => {
  // Update rust progression in real-time (unless manually controlled by debug slider)
  if (!useManualProgress) {
    const currentProgress = getProgress();
    applyProgress(currentProgress);
  } else {
    applyProgress(manualProgressValue);
  }
...
// --- Manual progress control for debug slider ---
export function setManualProgress(p) {
  useManualProgress = true;
  manualProgressValue = p;
}
